...
const [loading, setLoading] = useState(false);
  ...
  return (
    <>
      ...
      {/* Hero Banner */}
      <section
        className="relative w-full h-[50vh] md:h-[80vh] bg-cover bg-center"
        style={{
          backgroundImage:
      ...
      >
        <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
          <h1 className="px-3 bg-white font-serif text-[#B57E1F] text-md md:text-5xl p-2 rounded font-bold">
            {product.name}
          </h1>
        </div>
      </section>
      ...
      <section className=" w-full mx-auto px-4 py-16 space-y-16">
        ...
        <div className="flex flex-col lg:flex-row gap-15 items-center justify-center">
          ...
          <div className=" bg-white rounded-xl shadow-lg p-6 md:p-10">
            <h2 className="text-2xl  md:text-3xl font-semibold mb-6 text-[#B57E1F] border-b pb-2 font-serif">
              ...
              <table className="w-full  border-collapse">
                <tbody>
                  {product.specs?.map((spec, i) => (
                    <tr
                      key={i}
                      className={`${i % 2 === 0 ? "bg-white" : "bg-[#00537B]/20"} hover:bg-[#F7C600]`}
                    >
                      <td className="p-4  text-lg font-medium text-[#00537B] w-1/3">
                        {spec.label}
                      </td>
            ...
            <div className="flex flex-col md:flex-row gap-4 mt-6">
              <button
                ...
                className="flex-1 cursor-pointer text-center px-4 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-lg shadow hover:from-blue-600 hover:to-cyan-700 transition font-semibold"
              >
                Enquire Now
              </button>
              <a
        ...
        <div className="flex flex-col lg:flex-row gap-5">
          ...
          <div className="lg:w-2/2 bg-white rounded-xl shadow-lg p-6 md:p-10 overflow-y-auto max-h-[700px]">
            <h2 className="text-2xl md:text-4xl font-semibold text-[#B57E1F] mb-6 border-b pb-2 font-serif">
              Product Description
            </h2>
                ...
                product.description.map((block, i) => {
                  if (block.type === "h2")
                    return (
                      <h3
                        key={i}
                        className="text-xl md:text-2xl font-bold my-4 text-red-600"
                      >
                        {block.text}
                      </h3>
                    );
                  ...
                  if (block.type === "ul")
                    return (
                      <ul
                        key={i}
                        className="list-disc space-y-3 text-lg my-4 ml-6 text-black mb-4"
                      >
          ...
          <div className="lg:w-1/3 bg-[#00537B]/20 rounded-xl shadow-lg p-6 md:p-8 flex flex-col">
            <h2 className="text-2xl md:text-3xl font-semibold text-[#00537B] mb-6 border-b pb-2 text-center">
              Enquiry Form
            </h2>
...
Contact: ${phone}`;
  ...
  >
              ...
              <input
                type="hidden"
                name="_subject"
                value={`New Product Enquiry: ${product.name}`}
              />
            ...
            {(() => {
              ...
              return related.slice(0, 4).map((relatedProduct) => (
                ...
                >
                  ...
                  <h3 className="mt-4 text-lg font-semibold text-[#00537B]">
                    {relatedProduct.name}
                  </h3>
